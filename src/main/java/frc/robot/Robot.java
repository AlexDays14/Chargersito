...
public class Robot extends TimedRobot {
  ...
  public void autonomousPeriodic() {
    switch (m_autoSelected) {
      case kCustomAuto:
        ...
        if (m_timer.get() < 2.0) {
          Cdrive.curvatureDrive(0.5, 0.0, true); // drive forwards half speed
          if(m_timer.get() > 2.0 && m_timer.get() < 3.0){
            _Shooter.set(ControlMode.PercentOutput, 0.50);
          }
        } else {
          Cdrive.stopMotor(); // stop robot
          _Shooter.set(ControlMode.PercentOutput, 0);
        }
        break;
      case kDefaultAuto:
  ...
  public void teleopPeriodic() 
  {
    ...
    else if(_xboxOp.getRightBumper()){
      Shoot = false;
      Reverse = true;
    }
    else
